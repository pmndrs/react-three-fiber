/**
 * @react-three/tsl hooks against real roots from the @react-three/fiber/webgpu entry.
 * (Moved from fiber's tests/webgpu/index.test.tsx; the entry's own checks stay there.)
 */
/**
...
import { ReconcilerRoot, createRoot as createRootImpl, useThree, useTextures } from '@react-three/fiber/webgpu'
import { useUniform, useUniforms, useNodes, useLocalNodes } from '../src'
...
describe('WebGPU Hook Exports', () => {
  ...
  it('no longer exports the deprecated standalone utilities', async () => {
    const tsl = (await import('../src')) as Record<string, unknown>
    for (const name of [
      'removeUniforms',
      'clearScope',
      'clearRootUniforms',
