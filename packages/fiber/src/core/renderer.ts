...
function createRenderer<TCanvas>(_roots: Map<TCanvas, Root>, _getEventPriority?: () => any) {
  function createInstance(
    ...
    if (type === 'primitive') {
      if (props.object === undefined) throw new FiberError("Primitives without 'object' are invalid!")
      ...
      if (!target) {
        throw new FiberError(
          `${name} is not part of the THREE namespace! Did you forget to extend? See: https://docs.pmnd.rs/react-three-fiber/api/objects#using-3rd-party-objects-declaratively`,
        )
      }

      // Throw if an object or literal was passed for args
      if (!Array.isArray(args)) throw new FiberError('The args prop must be an array!')
