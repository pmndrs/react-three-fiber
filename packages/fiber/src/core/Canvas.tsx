...
/** Renderer classes and JSX constructors supplied by the entry point. */
...
function CanvasImpl({
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    if (effectiveSize.width > 0 && effectiveSize.height > 0 && canvas) {
      if (!root.current) {
        root.current = createRoot(canvas, provider)
...
/**
 * Internal DOM canvas bound to a renderer provider by each public entry.
