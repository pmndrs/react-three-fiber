...
To get started, make sure you have [Node](https://nodejs.org) (v22+) and [pnpm](https://pnpm.io) (v9+) installed. Install dependencies with:

```bash
pnpm install
```

This will automatically run `pnpm stub` to create development stubs that point to source code, allowing you to develop without rebuilding. See [BUILD](./docs/BUILD.md) for details on our build system.
...
```bash
pnpm changeset pre enter <alpha | beta | rc>
```
