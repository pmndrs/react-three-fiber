...
| gl              | Renderer props, an instance, or a sync/async factory. See [Renderers](#renderers).                                                                  | `{}`                                                       |
...
Canvas creates a `THREE.WebGLRenderer` by default. Use the `gl` prop to configure it or supply your own renderer. These options also apply to [createRoot](#createroot) through `root.configure({ gl })`.
...
| `gl` value               | Ownership                                                       |
| ------------------------ | --------------------------------------------------------------- |
| Omitted or a props object | R3F owns the renderer it creates and disposes it on unmount.      |
| A sync or async factory   | R3F owns the returned renderer and disposes it on unmount.        |
| A renderer instance      | You own the renderer and are responsible for calling `dispose()`. |
