...
Whenever you want to spread props or type components that rely on three elements, you can use the `ThreeElement` type to extract the mesh, group, or any other three element, including custom elements.

```tsx
import { ThreeElement } from '@react-three/fiber'

type Props = ThreeElement['mesh'] & { bar: boolean }

function Foo({ bar, ...props}: Props) {
  useEffect(() => {
    console.log(bar)
  }, [bar])
  return <mesh {...props} />
