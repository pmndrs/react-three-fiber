...
If you open your own root instead of using `<Canvas>`, you can extend the default elements with `extend`. The `* as THREE` namespace contains functions, numbers and strings alongside the classes; `extend` accepts it as-is and registers only the constructors, so no cast is needed. Extract the exact classes (`extend({ Mesh, Group, ... })`) when you want a smaller catalogue.
