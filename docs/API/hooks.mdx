...
| ----------------- | ----------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `renderer`        | The renderer instance — type depends on entry point (WebGL, WebGPU, or the union for the default entry) | `R3FRenderer`                                                                                                                                                       |
| `gl`              | **Deprecated** — alias of `renderer`, typed as `WebGLRenderer` for back-compat. Use `renderer`          | `THREE.WebGLRenderer`                                                                                                                                               |
| `scene`           | Scene                                                                         | `THREE.Scene`                                                                                                                                                                                                  |
| `camera`          | Camera                                                                        | `THREE.PerspectiveCamera`                                                                                                                                                                                      |
| `frustum`         | Camera frustum for visibility checks, auto-updated each frame                 | `THREE.Frustum`                                                                                                                                                                                                |
| `autoUpdateFrustum` | Whether frustum auto-updates each frame (default: true)                     | `boolean`                                                                                                                                                                                                      |
...
This hook allows you to execute code on every rendered frame, like running effects, updating controls, and so on. You receive the state (same as `useThree`) and the frame `delta`. The callback runs inside a shared render-loop; when the component unmounts it is unsubscribed automatically.
...
> [!NOTE]
> There is no `state.clock` in v10. Timing comes through the frame state instead: `state.delta` (seconds since last frame), `state.elapsed` (seconds since the first frame), `state.time` (the RAF timestamp), and `state.frame` (frame count). For example, `useFrame((state) => (mesh.rotation.y = state.elapsed))`.
...
`useFrame` accepts an options object for phase placement, ordering, fps throttling, and pause/resume — and the priority/render-takeover behavior below is part of a larger scheduler. See [Frame Loop & Scheduler](../frame-loop) for the full API.

### Taking over the render-loop

In v10 the render loop is phase-based. R3F runs its own default render job in the `render` phase. The moment you register **any** job in the `render` phase, R3F steps aside and that default render is skipped — it becomes your responsibility to render. The default render automatically resumes when your render-phase job unmounts.

```jsx
import { useFrame } from '@react-three/fiber'

function TakeOver() {
  // Registering a job in the 'render' phase disables the default render
...
To order callbacks within a phase, use `before`/`after` (by job id) or a numeric `priority`:

```jsx
useFrame(() => {}, { id: 'a', phase: 'update' })
useFrame(() => {}, { phase: 'update', after: 'a' }) // runs after job 'a'
```

> [!NOTE]
> A bare numeric second argument (`useFrame(fn, 1)`) is still accepted for ordering, but the explicit options object is preferred in v10. See [Frame Loop & Scheduler → Render takeover](../frame-loop#render-phase-takeover) for details.
