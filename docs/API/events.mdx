...
Also notice the `onPointerMissed` on the canvas element, which fires on clicks that haven't hit _any_ meshes. Similarly, `onDragOverMissed` and `onDropMissed` fire when drag/drop events don't hit any meshes.
...
<mesh
  ...
  onDragOver={(e) => console.log('dragging over')} // continuous drag event
  onDragOverEnter={(e) => console.log('drag entered')}
  onDragOverLeave={(e) => console.log('drag left')}
  onDragOverMissed={(e) => console.log('drag missed this object')}
  onDrop={(e) => console.log('dropped!', e.nativeEvent.dataTransfer?.files)}
  onDropMissed={(e) => console.log('drop missed this object')}
...
R3F supports drag and drop events, allowing you to drag files from your desktop onto 3D objects. This is useful for features like dropping textures onto meshes or importing models.

#### Drag events on meshes

| Event | Description |
|-------|-------------|
| `onDragOver` | Fires continuously while dragging over the object |
| `onDragOverEnter` | Fires once when drag enters the object |
| `onDragOverLeave` | Fires once when drag leaves the object |
| `onDragOverMissed` | Fires when drag is over canvas but misses this object |
| `onDrop` | Fires when a file/item is dropped on this object |
| `onDropMissed` | Fires when a drop misses this object |

#### Canvas-level missed events

The Canvas component accepts `onDragOverMissed` and `onDropMissed` props that fire when drag/drop events don't hit any objects:
...
When using drag and drop, you must prevent the browser's default behavior to stop it from opening dropped files:
...
<Canvas {...preventDefaults}>
```

#### Accessing dropped files

Dropped files are available via `e.nativeEvent.dataTransfer.files` on mesh events, or `e.dataTransfer.files` on Canvas missed events:
...
#### Example: Loading a dropped texture

Here's a complete example of loading a dropped image as a texture:

```jsx
function useFileTexture(file) {
...
function TexturedBox() {
  ...
  const texture = useFileTexture(file)
  
  return (
    <mesh
      onDrop={(e) => {
        const f = e.nativeEvent.dataTransfer?.files?.[0]
        if (f?.type.startsWith('image/')) setFile(f)
      }}
      onDragOverEnter={() => console.log('Drag over box')}
    >
      <boxGeometry />
      <meshBasicMaterial map={texture} color={texture ? 'white' : 'orange'} />
    </mesh>
  )
}
```
...
> [!NOTE]
> When loading textures from dropped files, remember to:
> - Set `colorSpace = THREE.SRGBColorSpace` for proper color display
> - Call `texture.dispose()` when replacing textures to free GPU memory
> - Use `URL.revokeObjectURL()` to clean up blob URLs
...
`setPointerCapture` and `releasePointerCapture` take a `pointerId` parameter like in the DOM. In v10 the event system tracks each pointer independently (an internal per-`pointerId` map), so multi-touch and multiple simultaneous captures are supported — each pointer's hover/capture state is isolated from the others.
...
### Prioritizing objects with `interactivePriority`

By default, intersections are sorted by distance from the camera (nearest first). Set `userData.interactivePriority` on an object to make it win regardless of distance — useful for gizmos, handles, or UI that must stay interactive even when occluded.
...
Objects with an `interactivePriority` are sorted ahead of objects without one; among several that have it, the higher value wins. Objects without it keep their normal distance-based order behind them.

### Frame-timed raycasting

For pointer-heavy scenes you can defer move-raycasts to the frame loop instead of running one per DOM pointer event. This is configured on the `events` prop and only applies while `frameloop="always"`:
...
When enabled, R3F batches pending move raycasts and flushes them once per frame; click events flush any pending move first so hover state stays accurate.

### XR pointers

> [!NOTE]
> **Preview.** WebXR controller/hand pointers register into the same event system via an internal pointer registry. The API surface is still settling — treat XR pointer support as a preview in v10.
