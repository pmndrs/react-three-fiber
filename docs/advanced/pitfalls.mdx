...
SetState is not suitable for fast updates. You don't want to involve React 60 or 120 times per second, even if in theory it could handle it. It comes down to this: it is not enough to just set a value, _you need managed frame deltas_, or else your project will run at different speeds depending on the end-users system refresh-rate, many updates in threejs need update flags (needsUpdate), or imperative functions.
