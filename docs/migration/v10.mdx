...
If you do want TSL or node materials, start the canvas on `WebGPURenderer` (`<Canvas renderer>`, or the `/webgpu` entry): the legacy `WebGLRenderer` cannot compile them, and R3F warns in development when a node material is attached without `renderer`. That does not require browser WebGPU support — `WebGPURenderer` falls back to WebGL2 automatically.
