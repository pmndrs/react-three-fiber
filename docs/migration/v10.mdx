...
```tsx
const setup = useCallback((mesh: THREE.Mesh | null) => mesh?.lookAt(0, 0, 0), [])
<mesh ref={setup} />
```
...
fires after uploading the texture to the GPU, and `<texture onUpdate={fn} />` now sets it. On an
object with no such property (a mesh, a group) the assignment is inert, and R3F logs a one-time
notice in case you were relying on the old hook.
