...
If you do want TSL or node materials, start the canvas on `WebGPURenderer` (`<Canvas renderer>`, or the `/webgpu` entry): the legacy `WebGLRenderer` cannot compile them, and without `renderer` a node material element throws with this fix in the error (a node material instance you attach gets a development warning). That does not require browser WebGPU support — `WebGPURenderer` falls back to WebGL2 automatically.
