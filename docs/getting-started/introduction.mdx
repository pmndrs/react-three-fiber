...
> [!IMPORTANT]
> You're reading the docs for **v10, currently in alpha**: APIs can still change before release. For the current stable release, see the [v9 docs](https://docs.pmnd.rs/react-three-fiber). The version picker at the bottom right of every page switches between them.

[![React Three Fiber banner](../banner-r3f.jpg)](/getting-started/examples)

```bash
npm install three @types/three @react-three/fiber@alpha
```
> [!WARNING]  
> Three-fiber is a React renderer, it must pair with a major version of React, just like react-dom, react-native, etc. @react-three/fiber@8 pairs with react@18, @react-three/fiber@9 and @10 pair with react@19.

> [!TIP]
> **New in v10:** WebGPU support and a rewritten frame-loop scheduler. See the [v10 Migration Guide](/migration/v10), the [WebGPU section](/webgpu/overview), and [Frame Loop & Scheduler](/frame-loop). React Native now lives in its own package, [`@react-three/native`](/getting-started/installation#react-native).
