...
> | `@react-three/fiber@10` (alpha) | `>=19.0 <19.4` | `>=0.185.0`  |
>
> The v10 bounds are deliberate — they state the versions we have tested (React 19.0 through 19.3),
> and move when a newer one is verified. Installing v10 against three r184 will fail peer resolution.
...
React Native support lives in its own package, `@react-three/native` (in v9 and earlier this was the `@react-three/fiber/native` entry point). It uses `expo-gl` and `expo-asset` under the hood for WebGL2 bindings and ensuring interplay between Metro and three.js loaders.
...
> [!NOTE]
> Make sure to import from `@react-three/native` or `@react-three/drei/native` for correct IntelliSense and platform-specific abstractions.
