...
> [!TIP]
> Looking to upgrade to the latest version? See the [v10 Migration Guide](/migration/v10) for WebGPU support and the new scheduler.
...
> **v10:** this section describes v9. In v10 the WebGPU renderer is `<Canvas renderer>` on the same `@react-three/fiber` import, with node materials as JSX and no `extend(THREE)` call. See the [WebGPU overview](../webgpu/overview) and the [v10 migration guide](../migration/v10).
...
`ThreeElements` was added as an interface since v8.1.0 and is the current way of declaring or accessing JSX within R3F since React's deprecation of `global` JSX (see https://react.dev/blog/2024/04/25/react-19-upgrade-guide#the-jsx-namespace-in-typescript). All JSX types belonging to R3F are accessible from `ThreeElements`.
...
`@react-three/test-renderer` now supports multiple entry points matching the fiber package structure. Choose the entry that matches your application's fiber import:

| Your Fiber Import | Test Renderer Import |
|-------------------|---------------------|
| `@react-three/fiber` | `@react-three/test-renderer` |
| `@react-three/fiber/legacy` | `@react-three/test-renderer/legacy` |
| `@react-three/fiber/webgpu` | `@react-three/test-renderer/webgpu` |

```diff
// If using WebGPU with TSL hooks
-import ReactThreeTestRenderer from '@react-three/test-renderer'
+import ReactThreeTestRenderer from '@react-three/test-renderer/webgpu'

// If using legacy WebGL-only
-import ReactThreeTestRenderer from '@react-three/test-renderer'
...
describe('WebGPU Hooks', () => {
  it('should test uniforms', async () => {
    let uniforms: any = null

    function TestComponent() {
      uniforms = useUniforms({ uTime: 0 })
      return null
    }
...
For more details, see the [WebGPU Testing Guide](https://github.com/pmndrs/react-three-fiber/blob/v10/packages/test-renderer/markdown/webgpu-testing.md).