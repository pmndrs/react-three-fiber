...
By default, Fiber sets up a render loop that renders the default `scene` from the default `camera` to the renderer (a `WebGLRenderer`, or a `WebGPURenderer` when you opt into the `renderer` prop).

In v10 the loop is driven by a single shared `requestAnimationFrame` scheduler, not three.js' `setAnimationLoop`. Work is organised into ordered **phases** — the defaults are `start`, `input`, `physics`, `update`, `render`, `finish` — and on each frame the scheduler:

1. Computes a shared frame `delta` and `elapsed`, so all jobs in the frame see the same timing
2. Runs every registered job in phase order (and, within a phase, by `before`/`after`/`priority`)
3. Runs the built-in render job in the `render` phase, which calls `renderer.render(scene, camera)` — unless a user job has taken over that phase

This single scheduler is shared across all roots/canvases, and `useFrame` is simply a job registered into it. For the full model — phases, ordering, fps throttling, render takeover, and demand mode — see [Frame Loop & Scheduler](/frame-loop).
