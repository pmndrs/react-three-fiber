...
Color management is now being handled by Three R139. We set `THREE.ColorManagement.legacyMode` to `false`. Now all colors will be converted from sRGB to linear space.
...
Here is a more generic setup with responsive resize:

```jsx
import { createRoot, events } from '@react-three/fiber'

// createRoot should only be called once
let root = createRoot(document.querySelector('canvas'))
// You can then configure it, same props that canvas normally takes
root.configure({ events })

function handleResize() {
  const width = window.innerWidth
  const height = window.innerHeight
  // Change configuration later on
